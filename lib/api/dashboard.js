...
export const fetchUser = async () => {
  try {
    const response = await fetch('https://litigate-backend.onrender.com/api/user', {
      headers: {
        ...getAuthHeaders(),
      },
    });
...
export const fetchDashboardStats = async (lawyerId) => {
  try {
    const response = await fetch(`https://litigate-backend.onrender.com/api/getStats/?lawyerId=${lawyerId}`, {
      headers: {
        ...getAuthHeaders(),
      },
    });
...
export const fetchNotifications = async (lawyerId) => {
  try {
    const response = await fetch(`https://litigate-backend.onrender.com/api/getNotifications/?lawyerId=${lawyerId}`, {
      headers: {
        ...getAuthHeaders(),
      },
    });
...
export const fetchEvents = async (lawyerId, date) => {
  try {
    const response = await fetch(`https://litigate-backend.onrender.com/api/events?lawyerId=${lawyerId}&date=${date}`, {
      headers: {
        ...getAuthHeaders(),
      },
    });
...
export const deleteEvent = async (eventId) => {
  try {
    const response = await fetch(`https://litigate-backend.onrender.com/api/events/${eventId}`, {
      method: "DELETE",
      headers: {
        ...getAuthHeaders(),
      },
